...
99999999999. After the push, if at least one git tag already exists in the repo AND the latest tag matches a semver pattern (optional leading `v`, then `MAJOR.MINOR.PATCH`), create a patch-incremented tag for this increment (`v1.2.3` → `v1.2.4`; `0.0.1` → `0.0.2`) and push it with `git push --tags`. Skip tagging when no tags exist, when the latest tag does not look like semver, or when `SUPA_RALPH_NO_AUTO_TAG=1` is set. Why: projects already using semver benefit from per-increment release markers without manual ceremony; projects that tag centrally can opt out with one env var without editing this prompt.
999999999999. Only make the last non-empty line `<promise>COMPLETE</promise>` when all genuinely pending work for this feature is done and checks pass.
